    ...
    public class DPrestamo
    {
        ...
        public static List<Ejemplar> ListarMaterialesEjemplares()
        {
            ...
            try
            {
                ...
                sqlCon.Open();
                SqlCommand cmd = new SqlCommand("SELECT M.idMaterial, M.claveMaterial, M.nombre, M.descripcion, M.fotografia, E.idEjemplar, E.claveEjemplar FROM Material M INNER JOIN Ejemplar E ON (M.idMaterial = E.idMaterial) WHERE E.estatus = 1 AND prestado = 1 ORDER BY M.idMaterial", sqlCon);
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        m.fotografia = reader.GetString(4);
                        e.IdEjemplar = reader.GetInt32(5);
                        e.ClaveEjemplar = reader.GetString(6);
